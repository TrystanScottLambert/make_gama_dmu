...
def bin_data(array_1: np.ndarray, array_2: np.ndarray, title: str) -> None:
    ...
    """
    n_bin = 100
    bins = np.linspace(np.min(array_1), np.max(array_1), n_bin)
    plt.hist(array_1, bins = bins, density=True, histtype='step')
    plt.hist(array_2, bins = bins, density=True, histtype='step')
    plt.title(title)
    plt.show()

def check_central_accuracy(ra_1: pd.Series, dec_1: pd.Series, ra_2:pd.Series, dec_2: pd.Series, title: str) -> None:
